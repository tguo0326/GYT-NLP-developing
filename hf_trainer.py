...
三个脚本 `imdb_bert_trainer.py` / `imdb_distilbert_trainer.py` /
`imdb_roberta_trainer.py` 只有「模型名 + 批大小 + 学习率」不同，
所以实现收在这里，各脚本只填一份配置。
...
4. `train_test_split(train, test_size=.2)` 没有 `random_state`，每次划分都不同，
   而且和 imdb_process.py 的划分不一致——三份 BERT 结果彼此、以及和 CNN/LSTM
...
def run(name: str, args: argparse.Namespace) -> dict | None:
    ...
    # 与 imdb_process.py 完全一致的划分，保证跨模型可比
