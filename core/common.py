...
ROOT = Path(__file__).resolve().parent.parent   # core/ 的上一级才是仓库根
...
@dataclass
class Bundle:
    """experiments/preprocess.py 产出的全部数据。字段顺序与 pickle 里的列表一一对应。"""
...
def load_data(path: Path = PICKLE_PATH) -> Bundle:
    if not path.exists():
        raise FileNotFoundError(
            f"找不到 {path}。请先运行：python experiments/preprocess.py"
        )
...
def write_submission(name: str, model: nn.Module, bundle: Bundle, device: torch.device) -> Path:
    ...
    1. **不能复用 `bundle.test_features`**。它是 `experiments/preprocess.py` 跑的时候按当时那份
